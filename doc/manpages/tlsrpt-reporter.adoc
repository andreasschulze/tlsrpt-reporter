...
The tlsrpt-fetcher associated with a tlsrpt-collectd transfers the pre-aggregated data to the tlsrpt-reporter.
...
Log a warning if the collectd clock reported by its fetcher differs by more than _sec_ seconds from the reporter´s clock.
The reporter starts querying the fetchers _sec_ seconds after the UTC day change, so a clock difference bigger than _sec_ seconds indicates the collectd might not yet been ready to provide the complete data for the previous day.
