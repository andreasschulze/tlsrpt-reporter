...
class MyTestCase(unittest.TestCase):
    ...
    def do_test(self, expect, co, fo, eo):
        self.reset(co, fo, eo)
        tmp = config.options_from_cmd_cfg_env(options_test, "/etc/none", self.section, self.envprefix)
        cres = ConfigTest(**tmp)
    ...
    def test_b0rkcfg(self, mock_stderr):
        with self.assertRaises(SystemExit):
            self.do_test({'ocfe': 'c', 'ocfx': "c", 'ocxe': "c", 'ocxx': "c", 'oxfe': "f", 'oxfx': "f", 'oxxe': "e", 'oxxx': "d"},
                         ...
                         {'ocfe': 'f', 'ocfx': "f", 'oxfe': "f", 'oxfx': "f"},
                         {'ocfe': 'e', 'ocxe': "e", 'oxfe': "e", 'oxxe': "e"})
        self.assertRegexpMatches(mock_stderr.getvalue(), r"dummy: error: unrecognized arguments: --b0rk_ocfe c")
    ...
    def test_intarg_cmd_string(self, mock_stderr):
        with self.assertRaises(SystemExit):
            self.do_test({'nr': 4, 'ocfe': 'c', 'ocfx': "c", 'ocxe': "c", 'ocxx': "c", 'oxfe': "f", 'oxfx': "f", 'oxxe': "e", 'oxxx': "d"},
                         ...
                         {'ocfe': 'f', 'ocfx': "f", 'oxfe': "f", 'oxfx': "f"},
                         {'ocfe': 'e', 'ocxe': "e", 'oxfe': "e", 'oxxe': "e"})
        self.assertRegexpMatches(mock_stderr.getvalue(), r"dummy: error: argument --nr: invalid int value: 'text'")
    ...
    def test_intarg_cmd_float(self, mock_stderr):
        with self.assertRaises(SystemExit):
            self.do_test(
                ...
                {'ocfe': 'f', 'ocfx': "f", 'oxfe': "f", 'oxfx': "f"},
                {'ocfe': 'e', 'ocxe': "e", 'oxfe': "e", 'oxxe': "e"})
        self.assertRegexpMatches(mock_stderr.getvalue(), r"dummy: error: argument --nr: invalid int value: '3.1415'")
